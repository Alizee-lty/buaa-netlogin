...
def run_as_root(action: str) -> int:
    ...
    command = [sys.executable, action] if _is_frozen() else [sys.executable, str(project / "main.py"), action]
    if not is_root():
...
def build_plist(runtime: Path, username: str, frozen: bool = False) -> Dict[str, Any]:
    return {
        "Label": LABEL,
        "ProgramArguments": ([str(runtime), "_watch"] if frozen else
                             [str(runtime), str(INSTALL_DIR / "main.py"), "_watch"]),
...
def _write_plist(runtime: Path, account: Any) -> None:
    ...
    _atomic_write(
        PLIST_PATH,
        plistlib.dumps(build_plist(runtime, account.pw_name, frozen=_is_frozen()), sort_keys=False),
        0o644,
    )
...
def install(source: Path, credentials: Dict[str, Any]) -> None:
    ...
    if source.resolve() == INSTALL_DIR.resolve() or (
        ...
        raise RuntimeError("请运行新下载的程序更新，不要直接从安装目录更新")
